...
d.findBy(function(h){return!Ext.isEmpty(h.filterName)});if(!Ext.isEmpty(f))for(var g=0;g<f.length;g++){this.filterFields[f[g].filterName]=f[g];e+=f[g].getHeight()}return d},setIsNotRendered:function(a){if(a.items)for(var b in a.items)this.setIsNotRendered(a.items[b]);else if(a.rendered)a.rendered=false},renderFilters:function(){if(this.fcc){for(var a in this.fcc){Ext.isIE&&this.setIsNotRendered(this.fcc[a]);this.renderFilterContainer(a,this.fcc[a])}this.setFilters(this.filters);this.highlightFilters(this.isFiltered());
this.focusedFilterName&&this.getFilterField(this.focusedFilterName).focus()}},onRender:function(){this.isFiltered()&&this.applyFilters(false);this.fireEvent("render",this)},getFilterField:function(a){return this.filterFields?this.filterFields[a]:null},setFilters:function(a,b){if((this.filters=a)&&this.filterFields){for(var d in this.filters)this.filterFields[d]||delete this.filters[d];for(d in this.filterFields){var e=this.filterFields[d],f=this.filters[e.filterName];if(Ext.isEmpty(f))b&&this.setFieldValue(e,
"");else this.setFieldValue(e,f)}}},onColResize:function(a,b){if(this.filterContainers){if(!this.inResizeProcess)if(this.grid.viewConfig&&this.grid.viewConfig.forceFit){this.onResize();return}var d=this.filterContainers[this.grid.getColumnModel().getColumnId(a)];if(d){if(isNaN(b))b=0;d.setWidth(b<3?0:b-3);d.doLayout(false,true)}}},onResize:function(){this.inResizeProcess=true;for(var a=this.grid.getColumnModel().getColumnCount(),b=0;b<a;b++){var d=this.grid.getView().getHeaderCell(b);d=Ext.get(d);
this.onColResize(b,d.getWidth())}this.inResizeProcess=false},onColHidden:function(a,b,d){if(!d){a=this.grid.getColumnModel().getColumnWidth(b);this.onColResize(b,a)}},onReconfigure:function(a,b,d){this.destroyFilters();this.configure(d);this.renderFilters()},saveFilters:function(a,b){var d={},e;for(e in this.filters)d[e]=this.filters[e];b.gridHeaderFilters=d;return true},loadFilters:function(a,b){var d=b.gridHeaderFilters;if(d)if(this.cfgFilterInit)this.filtersInitMode==="merge"&&Ext.apply(d,this.filters);
else this.filters=d},isFiltered:function(){for(var a in this.filters)if(!Ext.isEmpty(this.filters[a]))return true;return false},highlightFilters:function(a){if(this.highlightOnFilter)if(this.filterContainers)if(this.grid.getView().mainHd){var b=a?this.highlightColor:"transparent",d;for(d in this.filterContainers){var e=this.filterContainers[d];if(e.rendered)if(Ext.isEmpty(this.highlightCls))e.getEl().setStyle("backgroundColor",b);else a?e.getEl().addClass(this.highlightCls):e.getEl().removeClass(this.highlightCls)}}},
getFieldValue:function(a){return Ext.isFunction(a.filterEncoder)?a.filterEncoder.call(a,a.getValue()):a.getValue()},setFieldValue:function(a,b){if(Ext.isFunction(a.filterDecoder))b=a.filterDecoder.call(a,b);a.setValue(b)},applyFilter:function(a,b){if(arguments.length<2)b=true;if(a)if(a.isValid())if(!(a.disabled&&!Ext.isDefined(this.grid.store.baseParams[a.filterName]))){var d=this.getFieldValue(a);if(a.disabled||Ext.isEmpty(d)){delete this.grid.store.baseParams[a.filterName];delete this.filters[a.filterName]}else{this.grid.store.baseParams[a.filterName]=
d;this.filters[a.filterName]=d;if(this.ensureFilteredVisible){var e=this.grid.getColumnModel().getIndexById(a.columnId);e>=0&&this.grid.getColumnModel().isHidden(e)&&this.grid.getColumnModel().setHidden(e,false)}}this.highlightFilters(this.isFiltered());this.grid.fireEvent("filterupdate",a.filterName,d,a);a.startValue=d;b&&this.storeReload()}},applyFilters:function(a){if(arguments.length<1)a=true;for(var b in this.filterFields)this.applyFilter(this.filterFields[b],false);a&&this.storeReload()},storeReload:function(){if(this.grid.store.lastOptions){var a=
{start:0};if(this.grid.store.lastOptions.params&&this.grid.store.lastOptions.params.limit)a.limit=this.grid.store.lastOptions.params.limit;this.grid.store.load({params:a})}},getFilterContainer:function(a){return this.filterContainers?this.filterContainers[a]:null},destroyFilters:function(){if(this.filterFields)for(var a in this.filterFields){Ext.destroy(this.filterFields[a]);delete this.filterFields[a]}if(this.filterContainers)for(a in this.filterContainers){Ext.destroy(this.filterContainers[a]);
delete this.filterContainers[a]}}});Ext.ns("Ext.ux.grid");
...
[],p,q,t,s={},u={},v,x=0,y=b.length;x<y;x++){v=b[x];p=[];q=[];for(var z=x+e,w=0;w<f;w++){t=a[w];s.id=t.id;s.css=(w===0?"x-grid3-cell-first ":w==k?"x-grid3-cell-last ":"")+(this.cm.config[w].cellCls?" "+this.cm.config[w].cellCls:"");s.attr=s.cellAttr="";s.value=t.renderer(v.data[t.name],s,v,z,w,d);s.style=t.style;if(Ext.isEmpty(s.value))s.value="&#160;";if(this.markDirty&&v.dirty&&Ext.isDefined(v.modified[t.name]))s.css+=" x-grid3-dirty-cell";if(t.locked)q[q.length]=j.apply(s);else p[p.length]=j.apply(s)}t=
[];if(g&&(z+1)%2===0)t[0]="x-grid3-row-alt";if(v.dirty)t[1]=" x-grid3-dirty-row";u.cols=f;if(this.getRowClass)t[2]=this.getRowClass(v,z,u,d);u.alt=t.join(" ");u.cells=p.join("");u.tstyle=l;n[n.length]=h.apply(u);u.cells=q.join("");u.tstyle=m;o[o.length]=h.apply(u)}return[n.join(""),o.join("")]},processRows:function(a,b){if(!(!this.ds||this.ds.getCount()<1)){var d=this.getRows(),e=this.getLockedRows(),f,g;b=b||!this.grid.stripeRows;a=a||0;for(var h=0,j=d.length;h<j;++h){f=d[h];g=e[h];f.rowIndex=h;
...
if(a.isLocked(j))e[e.length]=b.apply(f);else d[d.length]=b.apply(f)}b=this.templates;f=[];h=[];a=this.cm;for(var k=this.rows,l="width:"+this.getTotalWidth()+";",m=0,n=k.length;m<n;m++){var o=k[m],p=[],q=[],t=j=0;for(g=o.length;j<g;j++){var s=o[j];s.colspan=s.colspan||1;var u=this.getColumnId(s.dataIndex?a.findColumnIndex(s.dataIndex):t),v=this.getGroupStyle.call(this,s,t);u=b.gcell.apply({cls:"ux-grid-hd-group-cell",id:u,row:m,style:"width:"+v.width+";"+(v.hidden?"display:none;":"")+(s.align?"text-align:"+
s.align+";":""),tooltip:s.tooltip?(Ext.QuickTips.isEnabled()?"ext:qtip":"title")+'="'+s.tooltip+'"':"",istyle:s.align=="right"?"padding-right:16px":"",btn:this.grid.enableHdMenu&&s.header,value:s.header||"&nbsp;"});if(a.isLocked(s.dataIndex?a.findColumnIndex(s.dataIndex):t))q[j]=u;else p[j]=u;t+=s.colspan}f[m]=b.header.apply({tstyle:l,cells:p.join("")});h[m]=b.header.apply({tstyle:l,cells:q.join("")})}f.push(b.header.apply({cells:d.join(""),tstyle:"width:"+this.getTotalWidth()+";"}));h.push(b.header.apply({cells:e.join(""),
...
d)},initComponent:function(){Ext.m3.GridPanel.superclass.initComponent.call(this);this.getStore().on("exception",this.storeException,this)},storeException:function(a,b,d,e,f){uiAjaxFailMessage(f,e)}});Ext.reg("gridpanel",Ext.m3.GridPanel);
...
this.getTotalWidth()+";"},q,t=0,s=b.length;t<s;t++){q=b[t];n=[];var u=t+e,v=this.renderRow(q,u,f,d,this.cm.getTotalWidth());if(v===false)for(v=0;v<f;v++){j=a[v];o.id=j.id;o.css=v==0?"x-grid3-cell-first ":v==l?"x-grid3-cell-last ":"";o.attr=o.cellAttr="";o.value=j.renderer.call(j.scope,q.data[j.name],o,q,u,v,d);o.style=j.style;if(Ext.isEmpty(o.value))o.value="&#160;";if(this.markDirty&&q.dirty&&typeof q.modified[j.name]!=="undefined")o.css+=" x-grid3-dirty-cell";if(j.id==this.grid.master_column_id){o.treeui=
this.renderCellTreeUI(q,d);j=h.mastercell}else j=h.cell;n[n.length]=j.apply(o)}else n.push(v);v=[];if(g&&(u+1)%2==0)v[0]="x-grid3-row-alt";if(q.dirty)v[1]=" x-grid3-dirty-row";p.cols=f;if(this.getRowClass)v[2]=this.getRowClass(q,u,p,d);p.alt=v.join(" ");p.cells=n.join("");p.display_style=d.isVisibleNode(q)?"":"display: none;";p.level=d.getNodeDepth(q);m[m.length]=k.apply(p)}return m.join("")},renderCellTreeUI:function(a,b){var d=this.templates.treeui,e=this.templates.elbow_line,f={},g,h,j=level=b.getNodeDepth(a);
...
Ext.m3.Window=Ext.extend(Ext.Window,{constructor:function(a,b){this.actionContextJson=this.parentWindow=null;if(b&&b.parentWindowID)this.parentWindow=Ext.getCmp(b.parentWindowID);if(b&&b.helpTopic)this.m3HelpTopic=b.helpTopic;if(b&&b.helpQtip)this.m3helpQtip=b.helpQtip;if(b&&b.contextJson)this.actionContextJson=b.contextJson;Ext.m3.Window.superclass.constructor.call(this,a)},initTools:function(){if(this.m3HelpTopic){var a=this.m3HelpTopic,b={id:"help",handler:function(){showHelpWindow(a)}};if(this.m3helpQtip)b.qtip=
this.m3helpQtip;this.addTool(b)}Ext.m3.Window.superclass.initTools.call(this)}});
...
method:"POST",params:this.actionContextJson,success:function(d){try{var e=smart_eval(d.responseText);if(e){e.on("closed_ok",function(f,g){this.fireEvent("afterselect",this,f,g)&&this.addRecordToStore(f,g)},this);b&&e.on("close",function(){b.hide()})}}finally{b&&b.hide()}},failure:function(){b&&b.hide();uiAjaxFailMessage.apply(this,arguments)},scope:this})}},addRecordToStore:function(a,b){var d=new Ext.data.Record,e=this.getValue();d.id=a;d[this.valueField]=a;d[this.displayField]=b;this.getStore().loadData({total:1,
...
this.getText()},showTriggers:function(a){if(a){var b;try{b=Ext.decode(this.getValue())}catch(d){b=this.getValue()}if(b){this.showClearBtn();this.showEditBtn()}this.showDictSelectBtn();this.showDropDownBtn()}else if(this.triggers){this.hideClearBtn();this.hideEditBtn();this.hideDictSelectBtn();this.hideDropDownBtn()}else this.hiddenTrigger4=this.hiddenTrigger3=this.hiddenTrigger2=this.hiddenTrigger1=true},setDisabled:function(a){this.disableTriggers(a);Ext.m3.AdvancedComboBox.superclass.setDisabled.call(this,
a);this.showTriggers(!a)},setReadOnly:function(a){var b=this.getWidth();if(b===0)b=this.bufferSize;Ext.m3.AdvancedComboBox.superclass.setReadOnly.call(this,a);this.disableTriggers(a);this.showTriggers(!a);if(a){this.el.setWidth(b);this.wrap&&this.wrap.setWidth(b)}else this.onResize(b)}});Ext.reg("m3-select",Ext.m3.AdvancedComboBox);
...
this,a);this.fireEvent("setToday",this,a)}};this.baseTriggers[1].hide=this.hideTriggerToday},onBlur:function(){this.beforeBlur();this.focusClass&&this.el.removeClass(this.focusClass);this.hasFocus=false;if(this.validationEvent!==false&&(this.validateOnBlur||this.validationEvent=="blur"))this.validate();var a=this.getValue();String(a)!==String(this.startValue)&&this.fireEvent("change",this,a,this.startValue);this.fireEvent("blur",this);this.postBlur()}});Ext.reg("m3-date",Ext.m3.AdvancedDataField);
Ext.m3.AdvancedScrollComboBox=Ext.extend(Ext.m3.AdvancedComboBox,{constructor:function(a,b){this.scrolled=0;Ext.m3.AdvancedScrollComboBox.superclass.constructor.call(this,a,b);this.bufferStore=new Ext.data.Store({model:this.store.model,recordType:this.store.recordType,proxy:this.store.proxy,reader:this.store.reader,sortInfo:this.store.sortInfo});this.bufferStore.on("load",this.onBufferLoad,this);this.elem=undefined},initComponent:function(){Ext.m3.AdvancedScrollComboBox.superclass.initComponent.call(this);
this.getStore().on("load",this.onStoreLoad,this)},onScroll:function(a,b){var d=Ext.DomQuery.selectNode(".x-combo-list-item",b),e=parseInt(this.defaultLimit)||0;a.stopPropagation();this.elem=b;if(d&&e>0&&this.store.getCount()!==this.store.getTotalCount()){d=(b.scrollTop+b.offsetHeight)/d.offsetHeight>>0;if(d-this.scrolled===e){this.scrolled=d;e=Ext.applyIf({start:d,limit:e},this.store.baseParams);this.bufferStore.load({params:e})}}},onBufferLoad:function(){var a=[],b,d;this.bufferStore.each(function(e){var f=
true;this.store.data.each(function(g){if(g===e.data)return f=false});f&&a.push(e)});this.store.add(a);b=Ext.DomQuery.jsSelect(".x-combo-list-item",this.elem);for(d in b)if(this.store.data.items[d]&&this.store.data.items[d].json)if(this.store.data.items[d]&&this.store.data.items[d].json&&b[d].className.indexOf(this.store.data.items[d].json.item_option)<0)b[d].className+=" "+this.store.data.items[d].json.item_option},onStoreLoad:function(){this.scrolled=0},onTriggerDropDownClick:function(){Ext.m3.AdvancedScrollComboBox.superclass.onTriggerDropDownClick.call(this);
this.innerList&&this.mon(this.innerList,"scroll",this.onScroll,this)}});Ext.ns("Ext.m3.CodeEditorConfig");Ext.apply(Ext.m3.CodeEditorConfig,{parser:{python:{mode:{name:"python",version:2,singleLineStringErrors:false}},css:{mode:"css"},html:{mode:"text/html",tabMode:"indent"},javascript:{mode:{name:"javascript",json:true}},sql:{lineNumbers:true,matchBrackets:true,indentUnit:4,mode:"text/x-plsql"}}});
...
"aftersubmit","submitfailed","closing_canceled","beforeloaddata","dataloaded")},getForm:function(){assert(this.formId,"\u041d\u0435 \u0437\u0430\u0434\u0430\u043d formId \u0434\u043b\u044f \u0444\u043e\u0440\u043c\u044b");return Ext.getCmp(this.formId).getForm()},getInvalidNames:function(a){var b=[],d,e,f,g,h,j;a.items.each(function(k){if(!k.validate()){d=true;e=k.el;for(h=[];d;)if(f=e.parent()){if(g=Ext.getCmp(f.id))if(g.getXType()=="panel")g.title&&h.push('"'+g.title+'"');else if(g.getXType()==
"m3-editwindow")d=false;e=f}else d=false;j="<br>- "+k.fieldLabel;if(h.length)j=j+" (\u043d\u0430 \u043f\u0430\u043d\u0435\u043b\u0438 "+h.join(" - ")+")";b.push(j)}});return b},showInvalidFields:function(a){Ext.Msg.show({title:"\u041f\u0440\u043e\u0432\u0435\u0440\u043a\u0430 \u0444\u043e\u0440\u043c\u044b",msg:"\u041d\u0430 \u0444\u043e\u0440\u043c\u0435 \u0438\u043c\u0435\u044e\u0442\u0441\u044f \u043d\u0435\u043a\u043e\u0440\u0440\u0435\u043a\u0442\u043d\u043e \u0437\u0430\u043f\u043e\u043b\u043d\u0435\u043d\u043d\u044b\u0435 \u043f\u043e\u043b\u044f:"+
a.toString()+".",buttons:Ext.Msg.OK,icon:Ext.Msg.WARNING})},submitForm:function(a,b,d){assert(this.formUrl,"\u041d\u0435 \u0437\u0430\u0434\u0430\u043d url \u0434\u043b\u044f \u0444\u043e\u0440\u043c\u044b");if(a=Ext.getCmp(this.formId).getForm()){b=this.getInvalidNames(a);if(b.length){this.showInvalidFields(b);return}}var e=new Ext.LoadMask(this.body,{msg:"\u0421\u043e\u0445\u0440\u0430\u043d\u0435\u043d\u0438\u0435..."}),f=Ext.applyIf(d||{},this.actionContextJson||{}),g=function(n){for(var o=[],
p=0;p<n.getCount();p++){var q=n.get(p);if(q.name&&q.localEdit)o.push(q);else if(q instanceof Ext.Container&&q.items!=undefined){q=g(q.items);o=o.concat(q)}}return o};d=g(this.items);for(b=0;b<d.length;b++){for(var h=d[b],j=h.getStore(),k=[],l=0;l<j.data.items.length;l++)k.push(j.data.items[l].data);f[h.name]=Ext.util.JSON.encode(k)}d=a.el.dom.elements||(document.forms[a.el.dom]||Ext.getDom(a.el.dom)).elements;var m;Ext.each(d,function(n){m=n.name;!n.disabled&&m&&delete f[m]});d={url:this.formUrl,
submitEmptyText:false,params:f,scope:this,success:function(n,o){try{if(this.fireEvent("aftersubmit",this,n,o)){this.fireEvent("closed_ok",o.response.responseText);this.close(true);smart_eval(o.response.responseText)}}finally{e.hide();this.disableToolbars(false)}},failure:function(n,o){this.fireEvent("submitfailed",this,n,o)&&uiAjaxFailMessage.apply(this,arguments);e.hide();this.disableToolbars(false)}};if(this.fireEvent("beforesubmit",d)){this.disableToolbars(true);e.show();a.submit(d)}},onChangeFieldValue:function(a,
b,d,e){if(a.originalValue!==b){a.isModified||e.changesCount++;a.isModified=true}else{a.isModified&&e.changesCount--;a.isModified=false}e.updateTitle();a.updateLabel()},clearModificationFlag:function(a){a=a||this;assert(a instanceof Ext.Container,"\u0414\u043e\u043b\u0436\u0435\u043d \u0431\u044b\u0442\u044c \u043a\u043e\u043d\u0442\u0435\u0439\u043d\u0435\u0440");this.changesCount=0;this.updateTitle();a.items.each(function(b){if(b instanceof Ext.form.Field&&b.isEdit){b.originalValue=b.getValue();
b.startValue=b.getValue();b.isModified=false;b.updateLabel()}else b instanceof Ext.Container&&this.clearModificationFlag(b)},this)},setFieldOnChange:function(a,b){if(a){if(a instanceof Ext.form.Field&&a.isEdit)a instanceof Ext.form.Checkbox?a.on("check",function(e,f){b.onChangeFieldValue(e,f,!f,b)}):a.on("change",function(e,f,g){b.onChangeFieldValue(e,f,g,b)});if(a.items)if(a.items instanceof Array)for(var d=0;d<a.items.length;d++)b.setFieldOnChange(a.items[d],b);else a.items.each(function(e){b.setFieldOnChange(e,
b)});if(a.titleItems)for(d=0;d<a.titleItems.length;d++)b.setFieldOnChange(a.titleItems[d],b)}},updateTitle:function(){if(this.title!==this.originalTitle&&this.originalTitle===null)this.originalTitle=this.title;this.changesCount!==0?this.setTitle("*"+this.originalTitle):this.setTitle(this.originalTitle)},close:function(a){if(this.changesCount!==0&&!a)this.fireEvent("beforeclose",this)!==false&&Ext.Msg.show({title:"\u0412\u043d\u0438\u043c\u0430\u043d\u0438\u0435",msg:"\u0414\u0430\u043d\u043d\u044b\u0435 \u0431\u044b\u043b\u0438 \u0438\u0437\u043c\u0435\u043d\u0435\u043d\u044b! C\u043e\u0445\u0440\u0430\u043d\u0438\u0442\u044c \u0438\u0437\u043c\u0435\u043d\u0435\u043d\u0438\u044f?",
buttons:Ext.Msg.YESNOCANCEL,fn:function(b){if(b==="yes")this.submitForm();else if(b==="no")this.hidden?this.doClose():this.hide(null,this.doClose,this);else this.fireEvent("closing_canceled")},animEl:"elId",icon:Ext.MessageBox.QUESTION,scope:this});else Ext.m3.EditWindow.superclass.close.call(this)},disableToolbars:function(a){for(var b=[this.getTopToolbar(),this.getFooterToolbar(),this.getBottomToolbar()],d=0;d<b.length;d++)b[d]&&b[d].setDisabled(a)},loadForm:function(){this.disableToolbars(true);
var a=new Ext.LoadMask(this.body,{msg:"\u0427\u0442\u0435\u043d\u0438\u0435 \u0434\u0430\u043d\u043d\u044b\u0445..."});a.show();if(this.fireEvent("beforeloaddata",this)){assert(this.dataUrl,"\u041d\u0435 \u0437\u0430\u0434\u0430\u043d dataUrl \u0434\u043b\u044f \u0444\u043e\u0440\u043c\u044b");this.getForm().doAction("load",{url:this.dataUrl,params:Ext.applyIf({isGetData:true},this.actionContextJson||{}),success:this.onSuccessLoadForm.createDelegate(this,[a],true),failure:this.onFailureLoadForm.createDelegate(this,
[a],true),scope:this})}},onSuccessLoadForm:function(a,b,d){var e,f,g,h=b.result.complex_data,j;for(j in h){e=a.findField(j);assert(e instanceof Ext.form.TriggerField,String.format("\u041f\u043e\u043b\u0435 {0} \u043d\u0435 \u043f\u0440\u0435\u0434\u043d\u0430\u0437\u043d\u0430\u0447\u0435\u043d\u043e \u0434\u043b\u044f \u0437\u0430\u0433\u0440\u0443\u0437\u043a\u0438 \u0434\u0430\u043d\u043d\u044b\u0445",j));if(f=h[j].id){g=new Ext.data.Record;g.set("id",f);g.set(e.displayField,h[j].value);e.getStore().add([g]);
e.setValue(f);e.collapse()}else e.clearValue()}d.hide();this.disableToolbars(false);this.fireEvent("dataloaded",b)},onFailureLoadForm:function(a,b,d){uiAjaxFailMessage.apply(this,arguments);d.hide();this.disableToolbars(false)}});Ext.ns("Ext.ux.grid");
...
"change"||Ext.isEmpty(this.applyMode))if(g.hasListener!=undefined){g.hasListener("change")||g.on("change",function(j){var k=j.getXType();k=="combo"||k=="datefield"||this.applyFilter(j)},this);g.hasListener("specialkey")||g.on("specialkey",function(j,k){k.stopPropagation();if(k.getKey()==k.ENTER)this.focusedFilterName=j.filterName},this);g.hasListener("select")||g.on("select",function(j){this.focusedFilterName=j.filterName;this.applyFilter(j)},this)}else g.listeners={change:function(j){var k=j.getXType();
if(!(k=="combo"||k=="datefield")){j.startValue=j.getValue();this.applyFilter(j)}},specialkey:function(j,k){k.stopPropagation();if(k.getKey()==k.TAB)this.focusedFilterName=undefined;if(k.getKey()==k.ENTER)this.focusedFilterName=j.filterName},select:function(j){this.focusedFilterName=j.filterName;this.applyFilter(j)},scope:this};else if(this.applyMode==="enter")g.listeners={specialkey:function(j,k){k.stopPropagation();if(k.getKey()==k.ENTER){this.focusedFilterName=j.filterName;this.applyFilters()}if(k.getKey()==
k.TAB)this.focusedFilterName=undefined},scope:this};var h=this.fcc[g.columnId];if(!h){h={cls:this.filterContainerCls,border:false,bodyBorder:false,bodyStyle:"background-color: transparent",labelSeparator:"",labelWidth:this.labelWidth,layout:"hbox",style:{},items:[]};this.containerConfig&&Ext.apply(h,this.containerConfig);this.fcc[g.columnId]=h}h.items.push({cls:"x-tree-filter-container",bodyStyle:"background-color: transparent",border:false,bodyBorder:false,layout:"form",padding:2,margins:"0 0 -4 0",
flex:1,items:[g]})}}},getHeaderCell:function(a){var b=this.tree.columns,d=this.tree.outerCt.query("colgroup"),e,f;f=0;d=this.tree.innerHd.query("td");for(len=d.length;f<len;f++){e=Ext.fly(d[f]);if(b[f].dataIndex==a)return e}return null},renderFilterContainer:function(a,b){if(!this.filterContainers)this.filterContainers={};for(var d=0,e=this.tree.columns.length;d<e;d++);d=this.getHeaderCell(a);d=Ext.get(d);if(Ext.isGecko)d.dom.style.MozUserSelect="text";d.dom.style.verticalAlign="top";b.width=d.getWidth()-
...
this.renderFilterContainer(a,this.fcc[a])}this.setFilters(this.filters);this.highlightFilters(this.isFiltered())}},onRender:function(){this.isFiltered()&&this.applyFilters(false);this.renderFilters();this.fireEvent("render",this)},getFilterField:function(a){return this.filterFields?this.filterFields[a]:null},setFilters:function(a,b){if((this.filters=a)&&this.filterFields){for(var d in this.filters)this.filterFields[d]||delete this.filters[d];for(d in this.filterFields){var e=this.filterFields[d],
f=this.filters[e.filterName];if(Ext.isEmpty(f))b&&this.setFieldValue(e,"");else this.setFieldValue(e,f)}}},onColResize:function(a,b){if(this.filterContainers){var d=this.filterContainers[this.tree.columns[a].dataIndex];if(d){if(isNaN(b))b=0;d.setWidth(b<3?0:b-3);d.doLayout(false,true)}}},onResize:function(){this.inResizeProcess=true;for(var a=this.tree.columns.length,b=0;b<a;b++){var d=this.getHeaderCell(this.tree.columns[b].dataIndex);d=Ext.get(d);this.onColResize(b,d.getWidth())}this.inResizeProcess=
false},onColHidden:function(a,b,d){if(!d){a=this.getHeaderCell(this.tree.columns[b].dataIndex).getWidth();this.onColResize(b,a);this.tree.updateColumnWidths()}},onReconfigure:function(a,b,d){this.destroyFilters();this.configure(d);this.renderFilters()},saveFilters:function(a,b){var d={},e;for(e in this.filters)d[e]=this.filters[e];b.gridHeaderFilters=d;return true},loadFilters:function(a,b){var d=b.gridHeaderFilters;if(d)if(this.cfgFilterInit)this.filtersInitMode==="merge"&&Ext.apply(d,this.filters);
...
getFieldValue:function(a){return Ext.isFunction(a.filterEncoder)?a.filterEncoder.call(a,a.getValue()):a.getValue()},setFieldValue:function(a,b){if(Ext.isFunction(a.filterDecoder))b=a.filterDecoder.call(a,b);a.setValue(b)},applyFilter:function(a,b){if(arguments.length<2)b=true;if(a)if(a.isValid()){var d=this.getFieldValue(a);if(a.disabled||Ext.isEmpty(d))delete this.filters[a.filterName];else this.filters[a.filterName]=d;this.highlightFilters(this.isFiltered());this.tree.fireEvent("filterupdate",a.filterName,
d,a);b&&this.storeReload()}},applyFilters:function(a){if(arguments.length<1)a=true;for(var b in this.filterFields)this.applyFilter(this.filterFields[b],false);a&&this.storeReload()},storeReload:function(){for(var a in this.filterFields)this.tree.actionContextJson&&delete this.tree.actionContextJson[a];this.tree.actionContextJson=Ext.applyIf(this.tree.actionContextJson||{},this.filters);this.tree.refreshStore();this.focusedFilterName&&this.getFilterField(this.focusedFilterName).focus()},getFilterContainer:function(a){return this.filterContainers?
...
this.cm,b=a.rows,d=this.templates,e=d.hcell,f=[],g=[],h=a.getColumnCount(),j=b.length,k="width:"+this.getTotalWidth()+";",l="width:"+this.getLockedWidth()+";",m=0;m<j;m++){for(var n=b[m],o=[],p=[],q=0,t=0;q<n.length;q++){var s=n[q],u=s.dataIndex?a.findColumnIndex(s.dataIndex):t;s.colspan=s.colspan||1;if(a.getColumnAt(u)!==undefined){u=this.getColumnId(u);var v=this.getGroupStyle(s,t);t+=s.colspan;s=d.gcell.apply({cls:"ux-grid-hd-group-cell",id:u,row:m,style:"width:"+v.width+";"+(v.hidden?"display:none;":
"")+(s.align?"text-align:"+s.align+";":""),tooltip:s.tooltip?(Ext.QuickTips.isEnabled()?"ext:qtip":"title")+'="'+s.tooltip+'"':"",istyle:s.align=="right"?"padding-right:16px":"",btn:this.grid.enableHdMenu&&s.header,value:s.header||"&nbsp;"});if(a.isLockedHdGroup(m,q))p[p.length]=s;else o[o.length]=s}}f[m]=d.header.apply({tstyle:k,cells:o.join("")});g[m]=d.header.apply({tstyle:l,cells:p.join("")})}b=[];m=[];for(q=0;q<h;q++)if(a.getColumnAt(q)!==undefined){n={};n.id=a.getColumnId(q);n.value=a.getColumnHeader(q)||
"";n.style=this.getColumnStyle(q,true);n.tooltip=this.getColumnTooltip(q);n.css=(q===0?"x-grid3-cell-first ":q==h-1?"x-grid3-cell-last ":"")+(a.config[q].headerCls?" "+a.config[q].headerCls:"");if(a.config[q].align=="right")n.istyle="padding-right:16px";else delete n.istyle;if(a.isLocked(q))m[m.length]=e.apply(n);else b[b.length]=e.apply(n)}f[j]=d.header.apply({tstyle:k,cells:b.join("")});g[j]=d.header.apply({tstyle:l,cells:m.join("")});return[f.join(""),g.join("")]},renderTotalSummary:function(){var a=
...
p=l.length;o<p;o++){m=l[o];if(d>=n&&d<n+m.colspan)break;n+=m.colspan}if(m&&m.header)if(a.hierarchicalColMenu){l="group-"+j+"-"+n;o=(o=f.items?f.getComponent(l):null)?o.menu:null;if(!o){o=new Ext.menu.Menu({itemId:l});o.on("itemclick",this.handleHdMenuClick,this);p=false;var q=true,t=n;for(n=n+m.colspan;t<n;t++){a.isHidden(t)||(p=true);if(a.config[t].hideable!==false)q=false}f.add({itemId:l,text:m.header,menu:o,hideOnClick:false,checked:p,disabled:q})}f=o}else h.push(m.header)}h.push(g);g=new Ext.menu.CheckItem({itemId:"col-"+
a.getColumnId(d),text:h.join(" "),checked:!a.isHidden(d),hideOnClick:false,disabled:a.config[d].hideable===false});a.fireEvent("beforeaddmenuitem",a,g);f.add(g)}}},getEditorParent:function(){return this.el.dom},getLockedRows:function(){if(!this.canGroup())return this.hasRows()?this.lockedBody.dom.childNodes:[];for(var a=[],b=this.getGroups(true),d,e=0,f=b.length,g,h;e<f;++e)if(d=b[e].childNodes[1]){d=d.childNodes;g=0;for(h=d.length;g<h;++g)a[a.length]=d[g]}return a},getLockedRow:function(a){return this.getLockedRows()[a]},
getTotalSummaryRow:function(a){return a===true?this.lockedTotalSummaryInner.firstChild.firstChild.firstChild.firstChild:this.totalSummaryInner.firstChild.firstChild.firstChild.firstChild},getCell:function(a,b){var d=this.cm.getLockedCount();if(b<d)return Ext.fly(this.getLockedRow(a)).query(this.cellSelector)[b];return Ext.grid.GridView.prototype.getCell.call(this,a,b-d)},addRowClass:function(a,b){var d=this.getLockedRow(a);d&&this.fly(d).addClass(b);Ext.grid.GridView.prototype.addRowClass.call(this,
a,b)},removeRowClass:function(a,b){var d=this.getLockedRow(a);d&&this.fly(d).removeClass(b);Ext.grid.GridView.prototype.removeRowClass.call(this,a,b)},removeRow:function(a){Ext.removeNode(this.getLockedRow(a));Ext.grid.GridView.prototype.removeRow.call(this,a)},removeRows:function(a,b){for(var d=this.lockedBody.dom,e=a;e<=b;e++)Ext.removeNode(d.childNodes[a]);Ext.grid.GridView.prototype.removeRows.call(this,a,b)},syncScroll:function(a){this.lockedScroller.dom.scrollTop=this.scroller.dom.scrollTop;
this.totalSummaryInner.scrollLeft=this.scroller.dom.scrollLeft;this.totalSummaryInner.scrollLeft=this.scroller.dom.scrollLeft;Ext.grid.GridView.prototype.syncScroll.call(this,a)},updateAllColumnWidths:function(){var a=this.getTotalWidth(),b=this.cm.getColumnCount(),d=this.getLockedWidth(),e=this.cm.getLockedCount(),f=[],g,h;this.updateLockedWidth();for(h=0;h<b;h++){f[h]=this.getColumnWidth(h);this.getHeaderCell(h).style.width=f[h]}var j=this.getLockedRows(),k=this.getRows(),l,m;h=0;for(g=k.length;h<
g;h++){l=j[h];l.style.width=d;if(l.firstChild){l.firstChild.style.width=d;l=l.firstChild.rows[0];for(m=0;m<e;m++)l.childNodes[m].style.width=f[m]}l=k[h];l.style.width=a;if(l.firstChild){l.firstChild.style.width=a;l=l.firstChild.rows[0];for(m=e;m<b;m++)l.childNodes[m-e].style.width=f[m]}}this.onAllColumnWidthsUpdated(f,a);this.syncHeaderHeight()},updateColumnWidth:function(a){var b=this.getColumnWidth(a),d=this.cm.getLockedCount(),e,f,g,h;this.updateLockedWidth();if(a<d){e=this.getLockedRows();f=this.getLockedWidth();
d=this.getTotalSummaryRow(true);g=a}else{e=this.getRows();f=this.getTotalWidth();g=a-d;d=this.getTotalSummaryRow(false)}this.getHeaderCell(a).style.width=b;for(var j=0,k=e.length;j<k;j++){h=e[j];h.style.width=f;if(h.firstChild){h.firstChild.style.width=f;h.firstChild.rows[0].childNodes[g].style.width=b}}d.childNodes[g].style.width=b;this.onColumnWidthUpdated(a,b,this.getTotalWidth());this.syncHeaderHeight()},updateColumnHidden:function(a,b){var d=this.cm.getLockedCount(),e,f,g,h,j=b?"none":"";this.updateLockedWidth();
if(a<d){e=this.getLockedRows();f=this.getLockedWidth();h=this.getTotalSummaryRow(true);d=a}else{e=this.getRows();f=this.getTotalWidth();h=this.getTotalSummaryRow(false);d=a-d}this.getHeaderCell(a).style.display=j;for(var k=0,l=e.length;k<l;k++){g=e[k];g.style.width=f;if(g.firstChild){g.firstChild.style.width=f;g.firstChild.rows[0].childNodes[d].style.display=j}}h.childNodes[d].style.display=j;this.onColumnHiddenUpdated(a,b,this.getTotalWidth());delete this.lastViewWidth;this.layout()},doRender:function(a,
b,d,e,f,g){if(b.length<1)return"";if(!this.canGroup()||this.isUpdating)return this.doRenderRows(a,b,d,e,f,g);var h=this.getGroupField(),j=this.cm.findColumnIndex(h),k,l="width:"+this.getTotalWidth()+";",m="width:"+this.getLockedWidth()+";";k=this.cm.config[j];var n=k.groupRenderer||k.renderer,o=this.showGroupName?(k.groupName||k.header)+": ":"",p=[],q,t,s,u;t=0;for(s=b.length;t<s;t++){var v=e+t,x=b[t],y=x.data[h];k=this.getGroup(y,x,n,v,j,d);if(!q||q.group!=k){u=this.constructId(y,h,j);this.state[u]=
!(Ext.isDefined(this.state[u])?!this.state[u]:this.startCollapsed);q={group:k,gvalue:y,text:o+k,groupId:u,startRow:v,rs:[x],cls:this.state[u]?"":"x-grid-group-collapsed"};p.push(q)}else q.rs.push(x);x._groupId=u}b=[];e=[];t=0;for(s=p.length;t<s;t++){k=p[t];k.style=l;this.doGroupStart(b,k,a,d,f,false);k.style=m;this.doGroupStart(e,k,a,d,f,true);h=this.doRenderRows(a,k.rs,d,k.startRow,f,g);b[b.length]=h[0];e[e.length]=h[1];this.doGroupEnd(b,k,a,d,f,false);this.doGroupEnd(e,k,a,d,f,true)}return[b.join(""),
e.join("")]},doRenderRows:function(a,b,d,e,f,g){var h=this.templates,j=h.cell;h=h.row;for(var k=f-1,l="width:"+this.getTotalWidth()+";",m="width:"+this.getLockedWidth()+";",n=[],o=[],p,q,t,s={},u={},v,x=0,y=b.length;x<y;x++){v=b[x];p=[];q=[];for(var z=x+e,w=0;w<f;w++){t=a[w];s.id=t.id;s.css=(w===0?"x-grid3-cell-first ":w==k?"x-grid3-cell-last ":"")+(this.cm.config[w].cellCls?" "+this.cm.config[w].cellCls:"");s.attr=s.cellAttr="";s.value=t.renderer.call(t.scope,v.data[t.name],s,v,z,w,d);s.style=t.style;
if(Ext.isEmpty(s.value))s.value="&#160;";if(this.markDirty&&v.dirty&&Ext.isDefined(v.modified[t.name]))s.css+=" x-grid3-dirty-cell";if(!Ext.isEmpty(v.error)&&Ext.isDefined(v.data[t.name])){var B=v.error.filter(function(A){return A.errorText&&A.column==t.name});if(!Ext.isEmpty(B)){s.css+=" x-grid3-row-invalid";s.css+=" x-form-invalid"}}if(t.locked)q[q.length]=j.apply(s);else p[p.length]=j.apply(s)}w=[];if(g&&(z+1)%2===0)w[0]="x-grid3-row-alt";if(v.dirty)w[1]=" x-grid3-dirty-row";u.cols=f;if(this.getRowClass)w[2]=
this.getRowClass(v,z,u,d);u.alt=w.join(" ");u.cells=p.join("");u.tstyle=l;n[n.length]=h.apply(u);u.cells=q.join("");u.tstyle=m;o[o.length]=h.apply(u)}return[n.join(""),o.join("")]},processRows:function(a,b){if(!(!this.ds||this.ds.getCount()<1)){var d=this.getRows(),e=this.getLockedRows(),f,g;b=b||!this.grid.stripeRows;a=a||0;for(var h=0,j=d.length;h<j;++h){f=d[h];g=e[h];f.rowIndex=h;g.rowIndex=h;if(!b){f.className=f.className.replace(this.rowClsRe," ");g.className=g.className.replace(this.rowClsRe,
" ");if((h+1)%2===0){f.className+=" x-grid3-row-alt";g.className+=" x-grid3-row-alt"}}this.syncRowHeights(f,g)}if(a===0){Ext.fly(d[0]).addClass(this.firstRowCls);Ext.fly(e[0]).addClass(this.firstRowCls)}Ext.fly(d[d.length-1]).addClass(this.lastRowCls);Ext.fly(e[e.length-1]).addClass(this.lastRowCls)}},syncRowHeights:function(a,b){if(this.syncHeights){var d=Ext.get(a),e=Ext.get(b);d.dom.style.height="";e.dom.style.height="";try{var f=d.dom.getBoundingClientRect().height,g=e.dom.getBoundingClientRect().height}catch(h){f=
d.getHeight();g=e.getHeight()}if(f>g)e.setHeight(f);else g>f&&d.setHeight(g)}},afterRender:function(){if(this.ds&&this.cm){var a=this.renderRows()||["&#160;","&#160;"];this.mainBody.dom.innerHTML=a[0];this.lockedBody.dom.innerHTML=a[1];this.processRows(0,true);this.deferEmptyText!==true&&this.applyEmptyText();this.grid.fireEvent("viewready",this.grid);this.grid.deferRowRender&&this.updateGroupWidths()}},layout:function(){if(this.mainBody){var a=this.grid,b=a.getGridEl().getSize(true),d=b.width;if(!(!a.hideHeaders&&
(d<20||b.height<20))){this.syncHeaderHeight();if(a.autoHeight){this.scroller.dom.style.overflow="visible";this.lockedScroller.dom.style.overflow="visible";if(Ext.isWebKit){this.scroller.dom.style.position="static";this.lockedScroller.dom.style.position="static"}}else{this.el.setSize(b.width,b.height);a=this.mainHd.getHeight();var e=this.totalSummary.getHeight();e=b.height-a-e}this.updateLockedWidth();if(this.forceFit){if(this.lastViewWidth!=d){this.fitColumns(false,false);this.lastViewWidth=d}}else{this.autoExpand();
this.syncHeaderScroll()}this.onLayout(d,e)}}},getOffsetWidth:function(){return this.cm.getTotalWidth()-this.cm.getTotalLockedWidth()+this.getScrollOffset()+"px"},updateHeaders:function(){var a=this.renderHeaders();this.innerHd.firstChild.innerHTML=a[0];this.innerHd.firstChild.style.width=this.getOffsetWidth();this.innerHd.firstChild.firstChild.style.width=this.getTotalWidth();this.lockedInnerHd.firstChild.innerHTML=a[1];a=this.getLockedWidth();this.lockedInnerHd.firstChild.style.width=a;this.lockedInnerHd.firstChild.firstChild.style.width=
...
l=0;n<m.length;n++){var o=m[n];try{var p=o.getBoundingClientRect().width}catch(q){p=o.getWidth()}l+=parseFloat(p)}Ext.isBorderBox||Ext.isWebKit&&!Ext.isSafari2&&!Ext.isChrome||(l=Math.max(l-this.borderWidth,0));k.style.width=l+"px"}}}}},updateTotalSummary:function(){var a=this.renderTotalSummary();this.totalSummaryInner.firstChild.innerHTML=a[0];this.totalSummaryInner.firstChild.style.width=this.getOffsetWidth();this.totalSummaryInner.firstChild.firstChild.style.width=this.getTotalWidth();this.lockedTotalSummaryInner.firstChild.innerHTML=
...
n,o,p,q;this.isUpdating=true;if(Ext.isNumber(a)){n=a;a=b.getAt(n)}else n=b.indexOf(a);if(!(!a||n<0)){for(q=0;q<d;q++){o=e[q];p=q==0?"x-grid3-cell-first":q==f?"x-grid3-cell-last ":"";p={id:o.id,style:o.style,css:p,attr:"",cellAttr:""};p.value=o.renderer.call(o.scope,a.data[o.name],p,a,n,q,b);if(Ext.isEmpty(p.value))p.value="&#160;";if(this.markDirty&&a.dirty&&Ext.isDefined(a.modified[o.name]))p.css+=" x-grid3-dirty-cell";if(!Ext.isEmpty(a.error)&&Ext.isDefined(a.data[o.name])){var t=a.error.filter(function(s){return s.errorText&&
s.column==o.name});if(!Ext.isEmpty(t)){p.css+=" x-grid3-row-invalid";p.css+=" x-form-invalid"}}if(o.locked)l[q]=m.apply(p);else k[q]=m.apply(p)}e=this.getRow(n);e.className="";f=this.getLockedRow(n);f.className="";this.grid.stripeRows&&(n+1)%2===0&&g.push("x-grid3-row-alt");if(this.getRowClass){h.cols=d;g.push(this.getRowClass(a,n,h,b))}this.fly(e).addClass(g).setStyle(h.tstyle);h.cells=k.join("");e.innerHTML=this.templates.rowInner.apply(h);this.fly(f).addClass(g).setStyle(j.tstyle);j.cells=l.join("");
...
this)&&Ext.Ajax.request({url:this.actionSelectUrl,method:"POST",params:this.actionContextJson,success:function(a){if(a=smart_eval(a.responseText)){a.initMultiSelect(this.checkedItems);a.on("closed_ok",function(b){this.addRecordsToStore(b);this.fireChangeEventOnDemand();this.fireEvent("select",this,this.checkedItems)},this)}},failure:function(){window.uiAjaxFailMessage.apply(this,arguments)},scope:this})},clearValue:function(){this.checkedItems.splice(0,this.checkedItems.length);this.view&&this.view.refresh();
this.setValue(this.getValue());this.fireChangeEventOnDemand()},addRecordsToStore:function(a){for(var b=0,d=[],e;b<a.length;b++){e=new Ext.data.Record;e.data[this.valueField]=a[b].data[this.valueField];e.data[this.displayField]=a[b].data[this.displayField];d.push(e)}this.checkedItems=d;this.view&&this.view.refresh();this.setValue(this.getValue())},findCheckedRecord:function(a){for(var b=0,d=-1;b<this.checkedItems.length;b++)if(this.checkedItems[b].data[this.valueField]===a.data[this.valueField]){d=
b;break}return d}});Ext.reg("m3-multiselect",Ext.m3.MultiSelectField);Ext.ns("Ext.m3");
Ext.m3.MultiSelectFilterColumn=Ext.extend(Ext.m3.MultiSelectField,{useOptAll:false,optAllId:-5,optAllOnLabel:"\u0421\u043d\u044f\u0442\u044c \u0432\u044b\u0434\u0435\u043b\u0435\u043d\u0438\u0435 \u0432\u0441\u0435\u0445 \u0437\u0430\u043f\u0438\u0441\u0435\u0439",divAllOn:"",optAllOffLabel:"\u0412\u044b\u0434\u0435\u043b\u0438\u0442\u044c \u0432\u0441\u0435 \u0437\u0430\u043f\u0438\u0441\u0438",divAllOff:"",initComponent:function(){Ext.m3.MultiSelectFilterColumn.superclass.initComponent.apply(this);if(this.useOptAll=
this.initialConfig.useOptAll){this.divAllOn="<div>"+this.optAllOnLabel+"</div>";this.divAllOff="<div>"+this.optAllOffLabel+"</div>";var a=Ext.decode(this.defaultValue),b,d;b=(d=a.length===this.store.data.items.length)?this.optAllOnLabel:this.optAllOffLabel;if(d){a.splice(0,0,this.optAllId);this.value=this.defaultValue=a=Ext.encode(a)}this.store.insert(0,this.store.reader.readRecords([[this.optAllId,b]]).records)}},initValue:function(){var a=0,b,d,e;if(this.store&&this.value&&this.mode==="local"){b=
Ext.util.JSON.decode(this.value);this.store.each(function(f){Ext.each(b,function(g){if(f.get(this.valueField)==g){this.checkedItems.push(f);this.refreshItem(f);return false}},this)},this)}else if(this.value)for(b=Ext.util.JSON.decode(this.value);a<b.length;a++){d=b[a];if(!(typeof d!=="object"||!(d[this.displayField]&&d[this.valueField]))){e=new Ext.data.Record;e.data[this.valueField]=d[this.valueField];e.data[this.displayField]=d[this.displayField];this.checkedItems.push(e)}}Ext.m3.MultiSelectField.superclass.initValue.call(this)},
fillPrevSavedValue:function(a){a=Ext.util.JSON.decode(a);var b=this.store.data.items.length-1,d=[];this.checkedItems.splice(0,this.checkedItems.length);this.useOptAll&&a.length===b&&this.checkedItems.push(this.store.getById(this.optAllId));for(b=0;b<a.length;b++){var e=this.store.getById(a[b]);if(e){this.checkedItems.push(e);d.push(e.id)}}return Ext.util.JSON.encode(d)},isOptAllChecked:function(){for(var a=false,b=0;b<this.checkedItems.length;b++)if(this.checkedItems[b].data[this.valueField]===this.optAllId){a=
true;break}return a},onTriggerDropDownClick:function(){Ext.m3.MultiSelectFilterColumn.superclass.onTriggerDropDownClick.call(this);if(this.useOptAll)this.view.all.elements[0].innerHTML=this.isOptAllChecked()?this.divAllOn:this.divAllOff},collapse:function(){if(this.isExpanded()){this.list.hide();Ext.getDoc().un("mousewheel",this.collapseIf,this);Ext.getDoc().un("mousedown",this.collapseIf,this);this.fireEvent("collapse",this);this.prevValue!=this.value&&this.fireEvent("select",this,this.checkedItems)}},
getText:function(){var a=[];Ext3.each(this.checkedItems,function(b){a.push(b.eqcode||b.get(this.displayField))},this);if(a.length>1&&this.multipleDisplayValue)return this.multipleDisplayValue;else{if(this.useOptAll){a.remove(this.optAllOnLabel);a.remove(this.optAllOffLabel)}return a.join(this.delimeter+" ")}},onSelect:function(a,b){if(a.data.id===this.optAllId){for(var d=this.isOptAllChecked(),e=this.store.data.items,f=0;f<e.length;f++){var g=e[f],h=this.findCheckedRecord(g);if(d&&h!==-1)this.checkedItems.remove(this.checkedItems[h]);
else h===-1&&this.checkedItems.push(g);this.refreshItem(g)}this.view.all.elements[0].innerHTML=d?this.divAllOff:this.divAllOn;this.setValue(this.getValue())}else this.selectCommonRecord(a,b)},selectCommonRecord:function(a,b){var d=this.store.getById(this.optAllId),e=this.findCheckedRecord(a);if(this.fireEvent("beforeselect",this,a,b)!==false){if(e===-1){this.checkedItems.push(a);if(this.useOptAll&&this.checkedItems.length===this.store.data.items.length-1){this.checkedItems.push(d);this.view.all.elements[0].innerHTML=
this.divAllOn}}else{this.checkedItems.remove(this.checkedItems[e]);if(this.useOptAll){if(this.checkedItems.indexOf(d)!==-1){this.checkedItems.remove(d);this.refreshItem(d)}this.view.all.elements[0].innerHTML=this.divAllOff}}this.refreshItem(a);this.setValue(this.getValue())}},getValue:function(){var a=[];Ext3.each(this.checkedItems,function(b){a.push(b.eqid||b.get(this.valueField))},this);this.useOptAll&&a.remove(this.optAllId);return Ext3.util.JSON.encode(a)},setValue:function(a){!a||a==="[]"?this.hideClearBtn():
this.showClearBtn();this.prevValue=this.value;this.value=this.getValue();this.setRawValue(this.getText());if(this.hiddenField)this.hiddenField.value=this.value;this.el&&this.el.removeClass(this.emptyClass)},triggerBlur:function(){this.focusClass&&this.el.removeClass(this.focusClass);this.wrap&&this.wrap.removeClass(this.wrapFocusClass);this.validate()}});Ext.reg("m3-multiselect-filter-column",Ext.m3.MultiSelectFilterColumn);Ext.ns("Ext.m3");
Ext.m3.MultiSelectWithInnerFilterField=Ext.extend(Ext.m3.MultiSelectField,{optAllId:-5,optAllLabel:"(\u0412\u044b\u0434\u0435\u043b\u0438\u0442\u044c \u0432\u0441\u0435)",prevCheckedItems:[],filterCheckedItems:[],filterApplyId:-1,optFilterApplyLabel:"\u0414\u043e\u0431\u0430\u0432\u0438\u0442\u044c \u0432\u044b\u0434\u0435\u043b\u0435\u043d\u043d\u044b\u0439 \u0444\u0440\u0430\u0433\u043c\u0435\u043d\u0442 \u0432 \u0444\u0438\u043b\u044c\u0442\u0440",optFilterAllLabel:"(\u0412\u044b\u0434\u0435\u043b\u0438\u0442\u044c \u0432\u0441\u0435 \u0440\u0435\u0437\u0443\u043b\u044c\u0442\u0430\u0442\u044b \u043f\u043e\u0438\u0441\u043a\u0430)",
checkboxImageWidth:16,dropDownListMinWidth:155,filterMode:false,resizable:true,innerListWidthConst:9,filterFieldDivWidthConst:17,initComponent:function(){this.checkedItems=[];if(!this.tpl){this.tpl='<tpl for="."><div ext:qtip="{'+this.displayField+'}" style="width: max-content;" class="x-combo-list-item x-multi-combo-item"><img src="'+Ext.BLANK_IMAGE_URL+'" class="{[this.getImgClass(values)]}" /><div style="padding-right:'+this.checkboxImageWidth+'px">{'+this.displayField+"}</div></div></tpl>";this.tpl=
new Ext.XTemplate(this.tpl,{getImgClass:this.getCheckboxCls.createDelegate(this)})}this.store.insert(0,this.store.reader.readRecords([[this.optAllId,this.optAllLabel]]).records);for(var a=0;a<this.store.data.items.length;a++){var b=this.store.data.items[a];this.checkedItems.push(b);this.prevCheckedItems.push(b)}Ext.m3.MultiSelectField.superclass.initComponent.apply(this)},findCheckedRecord:function(a,b){for(var d=-1,e=0;e<b.length;e++)if(b[e].data[this.valueField]===a.data[this.valueField]){d=e;break}return d},
onTriggerDropDownClick:function(){if(this.fireEvent("beforerequest",this)){if(this.isExpanded()){this.restorePreviousItems();this.collapse()}else{this.onFocus({});this.doQuery(this.allQuery,true)}this.el.focus()}},collapse:function(){if(this.isExpanded()){this.list.hide();this.fireEvent("collapse",this)}},okBtnHandler:function(){if(!this.filterOkBtn.disabled){if(this.filterMode){this.checkedItems.splice(0,this.checkedItems.length);for(var a=0;a<this.filterCheckedItems.length;a++){var b=this.filterCheckedItems[a];
b.data.id!==this.optAllId&&this.checkedItems.push(b)}this.filterMode=false;this.tpl.filterMode=false;this.filterField.dom.value="";this.toggleFilterFieldSpanIcon(true);this.view.refresh()}this.columnSpanTrigger=new Ext.Element(this.ownerCt.el.select("span").elements[0]);new Ext.ToolTip({width:160,target:this.columnSpanTrigger,title:"\u0420\u0435\u0437\u0443\u043b\u044c\u0442\u0430\u0442\u044b \u043e\u0442\u0444\u0438\u043b\u044c\u0442\u0440\u043e\u0432\u0430\u043d\u044b",mouseOffset:[-170,0]});this.prevCheckedItems=
[];for(a=0;a<this.checkedItems.length;a++)this.prevCheckedItems.push(this.checkedItems[a]);a=this.wrap.select("img").elements[1].id;Ext.get(a).addClass("x-form-filter-trigger");this.collapse();this.fireEvent("select",this,this.checkedItems)}},closeBtnHandler:function(){this.restorePreviousItems();this.collapse()},toggleFilterFieldSpanIcon:function(a){if(a){this.filterFieldTrigger.dom.childNodes[0].style.display="";this.filterFieldTrigger.dom.childNodes[1].style.display="none"}else{this.filterFieldTrigger.dom.childNodes[0].style.display=
"none";this.filterFieldTrigger.dom.childNodes[1].style.display=""}},restorePreviousItems:function(){this.filterField.dom.value="";this.toggleFilterFieldSpanIcon(true);this.filterMode=false;this.tpl.filterMode=false;this.checkedItems.splice(0,this.checkedItems.length);this.view.refresh();for(var a=0;a<this.prevCheckedItems.length;a++){var b=this.prevCheckedItems[a];this.checkedItems.push(b);this.refreshItem(b)}},closeIf:function(a){if(!this.isDestroyed&&!a.within(this.wrap)&&!a.within(this.list)){this.restorePreviousItems();
this.collapse()}},expand:function(){this.activateFilterOkBtn();if(!(this.isExpanded()||!this.hasFocus)){if(this.title||this.pageSize){this.assetHeight=0;if(this.title)this.assetHeight+=this.header.getHeight();if(this.pageSize)this.assetHeight+=this.footer.getHeight()}this.doResize(this.bufferSize>=this.dropDownListMinWidth?this.bufferSize:this.dropDownListMinWidth);this.list.alignTo.apply(this.list,[this.el].concat(this.listAlign));this.list.setZIndex(this.getZIndex());this.list.show();Ext.isGecko2&&
this.innerList.setOverflow("auto");this.mon(Ext.getDoc(),{scope:this,mousewheel:this.closeIf,mousedown:this.closeIf});this.fireEvent("expand",this)}},restrictHeight:function(){this.innerList.dom.style.height="";this.list.beginUpdate();this.list.setHeight(380);this.list.alignTo.apply(this.list,[this.el].concat(this.listAlign));this.list.endUpdate();this.innerList.setHeight(this.list.getHeight()-this.filterFieldWrapDiv.getHeight()-39);this.outerDiv.setHeight(this.list.getHeight())},selectByValue:function(a,
b){if(!Ext.isEmpty(a,true)){var d=this.findRecord(this.valueField||this.displayField,a);if(d){this.select(this.store.indexOf(d),b);return true}}return false},getCheckboxCls:function(a){for(var b=this.tpl.filterMode?this.filterCheckedItems:this.checkedItems,d=0;d<b.length;d++)if(a[this.valueField]==b[d].data[this.valueField])return"x-grid3-check-col-on";return"x-grid3-check-col"},onViewClick:function(a){var b=this.view.getSelectedIndexes()[0],d=(this.filterMode?this.filterStore:this.store).getAt(b);
d?this.onSelect(d,b):this.collapse();a!==false&&this.el.focus()},prepareFilterStoreData:function(a){for(var b=this.innerList.dom.childNodes.length,d=[{data:{id:this.optAllId,name:this.optFilterAllLabel}},{data:{id:this.filterApplyId,name:this.optFilterApplyLabel}}],e=[],f=0;f<b;f++)this.innerList.dom.lastChild.remove();for(f=0;f<d.length;f++){b=d[f];e.push([b.data.id,b.data.name])}for(f=0;f<this.store.data.items.length;f++){b=this.store.data.items[f];if(b.data.id!==this.optAllId&&b.data.name.toLocaleLowerCase().includes(a)){d.push(b);
e.push([b.data.id,b.data.name])}}this.filterStore=new Ext.data.Store({listeners:{},reader:new Ext.data.ArrayReader({idIndex:0},Ext.data.Record.create([{name:"id",mapping:0},{name:"name",mapping:1}])),data:e});for(f=0;f<d.length;f++){b=d[f];b.data.id!==this.filterApplyId&&this.filterCheckedItems.push(b)}return d},filterFieldOnKeyUp:function(a){a=a.target.value.toLocaleLowerCase();var b=this.view.getTemplateTarget(),d=[];if(a.length===0){this.checkedItems.length===0&&this.disableFilterOkBtn();d=this.store.getRange();
this.filterMode=false;this.tpl.filterMode=false;this.toggleFilterFieldSpanIcon(true)}else if(a.length===1){this.activateFilterOkBtn();this.toggleFilterFieldSpanIcon(false);return}else{this.filterCheckedItems.splice(0,this.filterCheckedItems.length);this.filterMode=true;this.tpl.filterMode=true;this.toggleFilterFieldSpanIcon(false);d=this.prepareFilterStoreData(a)}this.view.tpl.overwrite(this.innerList,this.view.collectData(d,0));this.view.all.fill(Ext.query(this.view.itemSelector,b.dom));this.view.updateIndexes(0)},
onClearFilterTriggerClick:function(){this.checkedItems.length===0&&this.disableFilterOkBtn();var a=this.store.getRange(),b=this.view.getTemplateTarget();this.filterMode=false;this.tpl.filterMode=false;this.view.tpl.overwrite(this.innerList,this.view.collectData(a,0));this.view.all.fill(Ext.query(this.view.itemSelector,b.dom));this.view.updateIndexes(0);this.filterField.dom.value="";this.toggleFilterFieldSpanIcon(true)},onResize:function(a){Ext.form.ComboBox.superclass.onResize.apply(this,arguments);
this.bufferSize=a},doResize:function(a){a=Math.max(a,this.minListWidth);this.list.setWidth(a);this.innerList.setWidth(a-this.list.getFrameWidth("lr")-this.innerListWidthConst);this.filterField.setWidth(this.filterFieldWrapDiv.getWidth()-this.filterFieldDivWidthConst)},initButtons:function(){this.buttonsDiv=this.outerDiv.createChild({style:"float: right; margin-top: 3px; margin-right: 3px;"});this.filterOkBtn=new Ext.Button({text:"\u041e\u041a",enableToggle:false,allowDepress:false,pressed:false,renderTo:this.buttonsDiv.id,
style:{display:"inline",marginRight:"5px"}});this.filterOkBtn.getEl().select("button").elements[0].style.width="50px";this.filterOkBtn.getEl().on("click",this.okBtnHandler,this);this.filterCloseBtn=new Ext.Button({text:"\u0417\u0430\u043a\u0440\u044b\u0442\u044c",enableToggle:false,allowDepress:false,pressed:false,renderTo:this.buttonsDiv.id,style:{display:"inline"}});this.filterCloseBtn.getEl().select("button").elements[0].style.width="70px";this.filterCloseBtn.getEl().on("click",this.closeBtnHandler,
this)},initFilterField:function(){this.filterField=this.outerDiv.createChild({tag:"input",type:"text",size:24,autocomplete:"off",cls:"x-form-text x-form-field",placeholder:"\u041f\u043e\u0438\u0441\u043a"});this.filterFieldWrapDiv=this.filterField.wrap({cls:"x-form-field-wrap x-form-field-trigger-wrap",style:"margin-bottom: 5px;"});this.filterFieldTrigger=this.filterFieldWrapDiv.createChild({tag:"span",cls:"x-form-twin-triggers",cn:[{tag:"img",cls:"x-form-trigger x-form-search-trigger",src:Ext.BLANK_IMAGE_URL},
{tag:"img",cls:"x-form-trigger x-form-clear-trigger",src:Ext.BLANK_IMAGE_URL}]});var a=this.filterFieldTrigger.dom.childNodes[1];a.style.display="none";this.mon(new Ext.Element(a),"click",this.onClearFilterTriggerClick,this,{preventDefault:true});this.filterField.on("keyup",this.filterFieldOnKeyUp,this)},initList:function(){if(!this.list){var a=Ext.getDom(this.getListParent()||Ext.getBody());this.list=new Ext.Layer({parentEl:a,shadow:this.shadow,cls:["x-combo-list",this.listClass].join(" "),constrain:false,
zindex:this.getZIndex(a)});this.list.setSize(this.listWidth||Math.max(this.wrap.getWidth(),this.minListWidth),0);this.list.swallowEvent("mousewheel");this.assetHeight=0;this.syncFont!==false&&this.list.setStyle("font-size",this.el.getStyle("font-size"));if(this.title){this.header=this.list.createChild({cls:"x-combo-list-hd",html:this.title});this.assetHeight+=this.header.getHeight()}this.outerDiv=this.list.createChild({style:"padding: 5px;"});this.initFilterField();this.innerList=this.outerDiv.createChild({cls:"x-combo-list-inner"});
this.innerList.dom.style.overflowX="scroll";this.innerList.dom.style.overflowY="scroll";this.mon(this.innerList,"mouseover",this.onViewOver,this);this.mon(this.innerList,"mousemove",this.onViewMove,this);this.initButtons();if(this.pageSize){this.footer=this.list.createChild({cls:"x-combo-list-ft"});this.pageTb=new Ext.PagingToolbar({store:this.store,pageSize:this.pageSize,renderTo:this.footer});this.assetHeight+=this.footer.getHeight()}this.view=new Ext.DataView({applyTo:this.innerList,tpl:this.tpl,
singleSelect:true,selectedClass:this.selectedClass,itemSelector:this.itemSelector||".x-combo-list-item",emptyText:this.listEmptyText,deferEmptyText:false});this.mon(this.view,{containerclick:this.onViewClick,click:this.onViewClick,scope:this});this.bindStore(this.store,true);if(this.resizable){this.resizer=new Ext.Resizable(this.list,{handles:"e",minWidth:this.dropDownListMinWidth});this.mon(this.resizer,"resize",function(b,d,e){this.maxHeight=e-this.handleHeight-this.list.getFrameWidth("tb")-this.assetHeight;
this.listWidth=d;this.innerList.setWidth(d-this.list.getFrameWidth("lr")-this.innerListWidthConst);this.filterField.setWidth(this.filterFieldWrapDiv.getWidth()-this.filterFieldDivWidthConst);this.restrictHeight()},this)}}},onTriggerClick:function(){return false},getText:function(){return""},viewRefreshNode:function(a,b){var d=this.view;if(a>-1){var e=d.isSelected(a),f=d.all.elements[a],g=d.bufferRender([b],a)[0];d.all.replaceElement(a,g,true);if(e){d.selected.replaceElement(f,g);d.all.item(a).addClass(d.selectedClass)}d.updateIndexes(a,
a)}},refreshItem:function(a){var b=this.filterMode?this.filterStore:this.store;this.view&&this.viewRefreshNode(b.indexOf(a),a)},filterSelect:function(a,b,d,e){if(a.data.id===this.optAllId){b=this.findCheckedRecord(a,e)!==-1;for(a=0;a<d.data.items.length;a++){var f=d.data.items[a],g=this.findCheckedRecord(f,e);if(f.data.id!==this.filterApplyId){if(b&&g!==-1)e.remove(e[g]);else g===-1&&e.push(f);this.refreshItem(f)}}}else if(a.data.id===this.filterApplyId){for(a=0;a<this.filterCheckedItems.length;a++){d=
this.filterCheckedItems[a];d.data.id!==this.optAllId&&this.checkedItems.indexOf(d)===-1&&this.checkedItems.push(d)}this.onClearFilterTriggerClick()}else this.selectCommonRecord(a,b,d,e)},disableFilterOkBtn:function(){this.filterOkBtn.disable()},activateFilterOkBtn:function(){this.filterOkBtn.setDisabled(false)},defaultSelect:function(a,b,d,e){if(a.data.id===this.optAllId){a=this.findCheckedRecord(a,e)!==-1;for(b=0;b<d.data.items.length;b++){var f=d.data.items[b],g=this.findCheckedRecord(f,e);if(a&&
g!==-1)e.remove(e[g]);else g===-1&&e.push(f);this.refreshItem(f)}}else this.selectCommonRecord(a,b,d,e);this.checkedItems.length===0?this.disableFilterOkBtn():this.activateFilterOkBtn()},onSelect:function(a,b){var d=this.filterMode?this.filterCheckedItems:this.checkedItems,e=this.filterMode?this.filterStore:this.store;this.filterMode?this.filterSelect(a,b,e,d):this.defaultSelect(a,b,e,d)},selectCommonRecord:function(a,b,d,e){var f=d.getById(this.optAllId),g=this.findCheckedRecord(a,e);if(this.fireEvent("beforeselect",
this,a,b)!==false){if(g===-1){e.push(a);e.length===d.data.items.length-1&&e.push(f)}else{e.remove(e[g]);if(e.indexOf(f)!==-1){e.remove(f);this.refreshItem(f)}}this.refreshItem(a);this.setValue(this.getValue())}},getValue:function(){var a=[];Ext3.each(this.checkedItems,function(b){a.push(b.eqid||b.get(this.valueField))},this);a.remove(this.optAllId);return Ext3.util.JSON.encode(a)},setValue:function(a){!a||a==="[]"?this.hideClearBtn():this.showClearBtn();this.prevValue=this.value;this.value=this.getValue();
this.setRawValue(this.getText());if(this.hiddenField)this.hiddenField.value=this.value;this.el&&this.el.removeClass(this.emptyClass)},triggerBlur:function(){this.focusClass&&this.el.removeClass(this.focusClass);this.wrap&&this.wrap.removeClass(this.wrapFocusClass);this.validate()}});Ext.reg("m3-multiselect-with-inner-filter-field",Ext.m3.MultiSelectWithInnerFilterField);Ext.namespace("Ext.ux");Ext.ux.NotificationMgr={notifications:[],originalBodyOverflowY:null};
...
Ext.isArray(e)?Ext.applyIf(b,e[0].json):Ext.applyIf(b,e.json);return b},onBeforeStateSave:function(){this.fireEvent("resize")}});Ext.reg("objectgrid",Ext.m3.ObjectGrid);
...
e.id;b[this.columnParamName]=this.getColumnModel().getDataIndex(d[1])}}if(a)b=Ext.isArray(e)?Ext.applyIf(b,e[0].json):Ext.applyIf(b,e.json);return b}});Ext.reg("editorobjectgrid",Ext.m3.EditorObjectGrid);Ext.ns("Ext.m3");
...
Ext.ux.PagingTreeNodeUI=Ext.extend(Ext.ux.tree.TreeGridNodeUI,{renderElements:function(a,b,d,e){this.indentMarkup=a.parentNode?a.parentNode.ui.getChildIndent():"";var f=a.attributes,g=parseInt(a.parentNode.attributes.page)||0,h=f.fullCount,j=a.parentNode.attributes.pageLimit||f.limit||25;a.parentNode.attributes.pageLimit=j;var k=Math.ceil(h/j);f=a.getOwnerTree().columns;var l=function(p,q,t,s){p.push('<div class="gs_tree_pgbtn');s&&p.push(" "+s);p.push('"><div gs:page="');p.push(q);p.push('" class="x-tree-col-text">');
...
q){var t=p.getOwnerTree().getLoader(),s=q.target;if(!s.attributes.getNamedItem("gs:page"))return false;var u=p.parentNode,v=u.attributes.pageLimit||25;s=s.attributes.getNamedItem("gs:page").value;t.baseParams.limit=v;t.baseParams.start=s*v;u.attributes.page=s;delete u.attributes.children;u.getUI().beforeLoad();t.load(u,function(){u.getUI().afterLoad();u.expand()});delete t.baseParams.start;delete t.baseParams.limit;return false});this.elNode=this.wrap.childNodes[0];this.ctNode=this.wrap.childNodes[1];
...
Ext.destroy(this.previewTip)}});Ext.ux.Lightbox.register("a[rel^=lightbox]");Ext.reg("imageuploadfield",Ext.ux.form.ImageUploadField);function createGridPanel(a,b){return a.editor?new Ext.m3.EditorGridPanel(a,b):new Ext.m3.GridPanel(a,b)}function createObjectGrid(a,b){return a.editor?new Ext.m3.EditorObjectGrid(a,b):new Ext.m3.ObjectGrid(a,b)}function createObjectTree(a,b){return new Ext.m3.ObjectTree(a,b)}function createAdvancedComboBox(a,b){return new Ext.m3.AdvancedComboBox(a,b)}
function createAdvancedScrollComboBox(a,b){return new Ext.m3.AdvancedScrollComboBox(a,b)}function createAdvancedDataField(a,b){return new Ext.m3.AdvancedDataField(a,b)}
...
Ext.override(Ext.Element,{insertAfter:function(a){(a=Ext.getDom(a))&&a.parentNode&&a.parentNode.insertBefore(this.dom,a.nextSibling);return this}});
...
Ext.override(Ext.form.Field,{setReadOnly:function(a){if(a||!(this.ownerCt&&this.ownerCt.readOnly)){setReadOnlyField.call(this,a);restoreClass.call(this,a)}}});var setReadOnlyTriggerField=Ext.form.TriggerField.prototype.setReadOnly;Ext.override(Ext.form.TriggerField,{setReadOnly:function(a){if(a||!(this.ownerCt&&this.ownerCt.readOnly)){setReadOnlyTriggerField.call(this,a);restoreClass.call(this,a)}}});
Ext.override(Ext.grid.GridView,{fitColumns:function(a,b,d){b=this.cm;var e=b.getTotalWidth(false),f=this.getGridInnerWidth(),g=f-e;e=[];var h=0,j=0,k,l;if(f<20||g===0)return false;g=b.getColumnCount(true);var m=b.getColumnCount(false),n=g-(Ext.isNumber(d)?1:0);if(n===0){n=1;d=undefined}for(l=0;l<m;l++)if(!b.isFixed(l)&&l!==d){k=b.getColumnWidth(l);e.push(l,k);if(!b.isHidden(l)){h=l;j+=k}}for(j=(f-b.getTotalWidth())/j;e.length;){k=e.pop();l=e.pop();b.setColumnWidth(l,Math.floor(k+k*j),true)}e=b.getTotalWidth(false);
if(e>f){d=n==g?h:d;f=Math.max(1,b.getColumnWidth(d)-(e-f));b.setColumnWidth(d,f,true)}a!==true&&this.updateAllColumnWidths();return true}});Ext.override(Ext.Element,{addStyles:function(a,b){var d=0,e=a.match(/\w/g),f,g,h=e.length;for(g=0;g<h;g++)if(f=(f=e[g])&&parseFloat(this.getStyle(b[f])))d+=Math.abs(f);return d}});
function beforeColMenuShow(){var a=this.cm,b=this.cm.rows,d,e,f,g,h,j;this.colMenu.removeAll();for(var k=0,l=a.getColumnCount();k<l;k++){d=this.colMenu;j=a.getColumnHeader(k);e=[];if(a.config[k].fixed!==true&&a.config[k].hideable!==false){for(var m=0,n=b.length;m<n;m++){h=b[m];for(var o=g=0,p=h.length;o<p;o++){f=h[o];if(k>=g&&k<g+f.colspan)break;g+=f.colspan}if(f&&f.header)if(a.hierarchicalColMenu){h="group-"+m+"-"+g;o=(o=d.items?d.getComponent(h):null)?o.menu:null;if(!o){o=new Ext.menu.Menu({itemId:h});
o.on("itemclick",this.handleHdMenuClick,this);p=false;var q=true,t=g;for(g=g+f.colspan;t<g;t++){a.isHidden(t)||(p=true);if(a.config[t].hideable!==false)q=false}d.add({itemId:h,text:f.header,menu:o,hideOnClick:false,checked:p,disabled:q})}d=o}else e.push(f.header)}e.push(j);e=new Ext.menu.CheckItem({itemId:"col-"+a.getColumnId(k),text:e.join(" "),checked:!a.isHidden(k),hideOnClick:false,disabled:a.config[k].hideable===false});a.fireEvent("beforeaddmenuitem",a,e);d.add(e)}}}
Ext.override(Ext.ux.grid.ColumnHeaderGroup,{init:function(a){this.viewConfig.beforeColMenuShow=beforeColMenuShow;Ext.applyIf(a.colModel,this.config);Ext.apply(a.getView(),this.viewConfig)}});
Ext.override(Ext.Button,{onClick:function(a){var b=Date.now();if(b<this.lastClickTimestamp+500)this.lastClickTimestamp=b;else{this.lastClickTimestamp=b;a&&a.preventDefault();if(a.button===0)if(!this.disabled){this.doToggle();this.menu&&!this.hasVisibleMenu()&&!this.ignoreNextClick&&this.showMenu();this.fireEvent("click",this,a);if(this.handler)this.handler.call(this.scope||this,this,a)}}}});
Ext.override(Ext.m3.AdvancedDataField,{onBlur:function(){if(this.value!=this.getRawValue()){this.beforeBlur();this.focusClass&&this.el.removeClass(this.focusClass);this.hasFocus=false;if(this.validationEvent!==false&&(this.validateOnBlur||this.validationEvent=="blur"))this.validate();var a=this.getValue();String(a)!==String(this.startValue)&&this.fireEvent("change",this,a,this.startValue);this.fireEvent("blur",this);this.postBlur()}}});
